...
func wgRequestSubIP(ip string, publicKey string, uuid string) string {
 AndroidLogger{level: C.ANDROID_LOG_DEBUG, tag: "WireGuard"}.Printf("wgRequestSubIP: %s %s %s", ip, publicKey, uuid)
...
func goWgRequestSubIP(ip string, publicKey string, uuid string) string {
 ...
 AndroidLogger{level: C.ANDROID_LOG_DEBUG, tag: "WireGuard"}.Printf("wgRequestSubIP: %s", resp.AllowedIP)
